...
public sealed class KineticaIngestor<T>
{
    ...
    /// and the number of batches flushed across all worker queues.
    /// </returns>
    ...
    public IngestionResult Flush()
    {
        ...
        foreach ( Utils.WorkerQueue<T> workerQueue in this.workerQueues )
        {
            ...
            var result = flush( queue, workerQueue.Url );
    ...
    /// <summary>
    /// Inserts the given list of records into the worker at <paramref name="url"/>.
    /// Returns an <see cref="IngestionResult"/> with the server-reported counts, or
    /// a zero result when the queue is empty.
    /// </summary>
    /// <param name="queue">The list of records to insert.</param>
    /// <param name="url">The address of the Kinetica worker.</param>
    /// <returns>
    /// An <see cref="IngestionResult"/> containing the inserted/updated counts
    /// reported by the server, and <see cref="IngestionResult.FlushedBatches"/> of
    /// 1 when a batch was submitted, 0 when the queue was empty.
    /// </returns>
    /// <exception cref="InsertException">Thrown on any delivery failure.</exception>
    private IngestionResult flush( IList<T> queue, System.Uri url )
    {
        if ( queue.Count == 0 )
...
/// <summary>
/// Result returned by <see cref="KineticaIngestor{T}.Flush"/> and
/// <see cref="KineticaIngestor{T}.FlushAsync"/>.
/// </summary>
