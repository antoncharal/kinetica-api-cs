...
public sealed class KineticaType
{
    ...
    public Schema? AvroSchema => _schema;
    ...
    public bool ContainsColumn(string name) => _columnMap.ContainsKey(name);

    /// <inheritdoc cref="Columns"/>
    [Obsolete("Use Columns property instead.")]
    public IList<Column> getColumns() { return _columns; }
    ...
    public Type? getSourceType() { return _sourceType; }

    /// <inheritdoc cref="SchemaString"/>
    [Obsolete("Use SchemaString property instead.")]
    public string getSchemaString() { return _schemaString; }
    ...
    public string create(Kinetica kinetica)
    {
        ...
        if ( this._sourceType != null )
            kinetica.SetKineticaSourceClassToTypeMapping( this._sourceType, this );

        // Register the type with Kinetica
        CreateTypeResponse response = kinetica.createType( _schemaString, _label, _properties);
    ...
    private void Initialize()
    {
        ...
        {
            string columnName = _columns[i].Name;
                                         ...
                                         IDictionary<string, IList<string>> properties = null)
    {
        ...
        try
        {
            _schema = RecordSchema.Parse(typeSchema);
        }
        catch (Exception ex)
        {
            throw new KineticaException(ex.ToString());
        ...
        foreach (var field in fields)
        {
            ...
            _columnMap[fieldName] = _columns.Count - 1;
        }
    ...
    private void CreateSchema()
    {
        ...
        if (_schemaString != null)
        {
            try
            {
                _schema = RecordSchema.Parse(_schemaString);
                return;
            }
            catch (Exception ex)
            {
        ...
        try
        {
            _schema = RecordSchema.Parse(schemaString);
        }
        catch (Exception ex)
        {
            throw new KineticaException(ex.ToString());
        ...
        _schemaString = _schema.ToString();
        return;
    }  // end CreateSchema()
