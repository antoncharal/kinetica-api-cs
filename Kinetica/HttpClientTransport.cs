...
internal sealed class HttpClientTransport : IHttpTransport, IDisposable
{
    ...
    public HttpClientTransport(TimeSpan timeout, bool version11Only = true)
    {
        _httpVersion = version11Only ? HttpVersion.Version11 : HttpVersion.Version20;
    ...
    {
        using var request = BuildRequest(url, body, contentType, authorization, _httpVersion);
        ...
        Version httpVersion)
    {
        var request = new HttpRequestMessage(HttpMethod.Post, url)
        {
            ...
            // Force HTTP/1.1 by default to preserve existing behaviour — Kinetica servers
            // typically do not support HTTP/2 and mis-negotiation would break the
            // connection silently.  Callers may opt into HTTP/2+ via Kinetica.Options.Version11HttpOnly.
            Version = httpVersion,
        };
