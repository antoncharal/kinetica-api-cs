...
/// these record keys.
/// </summary>
internal sealed partial class RecordKey
{
    /// <summary>Matches YYYY-MM-DD dates.</summary>
    [System.Text.RegularExpressions.GeneratedRegex(@"\A(\d{4})-(\d{2})-(\d{2})$")]
    private static partial Regex DateRegex();

    /// <summary>Matches YYYY-MM-DD [HH:MM:SS[.mmm]] datetimes.</summary>
    [System.Text.RegularExpressions.GeneratedRegex(@"\A(?<year>\d{4})-(?<month>\d{2})-(?<day>\d{2})(?<time>\s+(?<hour>\d{1,2}):(?<min>\d{2}):(?<sec>\d{2})(?:\.(?<ms>\d{1,6}))?)?$")]
    private static partial Regex DateTimeRegex();

    /// <summary>Matches decimal numbers with up to 19 digits of precision and 4 digits of scale.</summary>
    [System.Text.RegularExpressions.GeneratedRegex(@"\A\s*(?<sign>[+-]?)((?<int>\d+)(\.(?<intfrac>\d{0,4}))?|\.(?<onlyfrac>\d{1,4}))\s*\z")]
    private static partial Regex DecimalRegex();

    /// <summary>Matches IPv4 addresses in xxx.xxx.xxx.xxx format (xxx in [0, 255]).</summary>
    [System.Text.RegularExpressions.GeneratedRegex(@"\A(?<a>\d{1,3})\.(?<b>\d{1,3})\.(?<c>\d{1,3})\.(?<d>\d{1,3})$")]
    private static partial Regex IPv4Regex();

    /// <summary>Matches HH:MM:SS[.mmm] times.</summary>
    [System.Text.RegularExpressions.GeneratedRegex(@"\A(?<hour>\d{1,2}):(?<minute>\d{2}):(?<seconds>\d{2})(\.(?<milliseconds>\d{1,3}))?$")]
    ...
    public void addDecimal(string value)
    {
        ...
        Match match = DecimalRegex().Match(value);
        if (!match.Success)
        {
            // No match, so the key is invalid
            this.is_valid = false;
    ...
    public void addIPv4(string value)
    {
        ...
        Match match = IPv4Regex().Match(value);
        if (!match.Success)
        {
            // No match, so the key is invalid
            this.is_valid = false;
